...
@generated tail_params(::Type{T}) where {T} = :($(Tuple{Base.tail((T.parameters...,))...}))
...
rmaptype_Tuple(fn::F, ::Type{T}) where {F, E, T <: Tuple{E}} = (rmaptype(fn, first_param(T)),)
...
rmaptype_Tuple(fn::F, ::Type{T1}, ::Type{T2}) where {F, T1 <: Tuple, T2 <: Tuple} =
    (rmaptype(fn, first_param(T1), first_param(T2)), rmaptype_Tuple(fn, tail_params(T1), tail_params(T2))...)
...
rmap(fn::F, X::Tuple) where {F} = (rmap(fn, first(X)), rmap(fn, Base.tail(X))...)
...
rmap(fn::F, X::Tuple{}, Y::Tuple) where {F} = ()
rmap(fn::F, X::Tuple, Y::Tuple{}) where {F} = ()
rmap(fn::F, X::Tuple, Y::Tuple) where {F} = (rmap(fn, first(X), first(Y)), rmap(fn, Base.tail(X), Base.tail(Y))...)
...
rmaptype(fn::F, ::Type{T}) where {F, names, Tup, T <: NamedTuple{names, Tup}} = NamedTuple{names, rmaptype(fn, Tup)}

rmaptype(fn::F, ::Type{T1}, ::Type{T2}) where {F, T1, T2} = fn(T1, T2)
rmaptype(fn::F, ::Type{T1}, ::Type{T2}) where {F, T1 <: Tuple, T2 <: Tuple} = Tuple{rmaptype_Tuple(fn, T1, T2)...}
rmaptype(
    fn::F,
    ::Type{T1},
    ::Type{T2},
) where {F, names, Tup1, Tup2, T1 <: NamedTuple{names, Tup1}, T2 <: NamedTuple{names, Tup2}} =
...
rzero(::Type{T}) where {T <: Tuple} = (rzero(first_param(T)), rzero(tail_params(T))...)
rzero(::Type{Tup}) where {names, T, Tup <: NamedTuple{names, T}} = NamedTuple{names}(rzero(T))
...
rconvert(::Type{T}, X) where {T} = rmap((zero_value, x) -> convert(typeof(zero_value), x), rzero(T), X)
